import { IJdGoodsListModel } from "@/apis/model/taoKeModel"
import { showJdjxByEliteId } from "@/apis/taoKe"
 ...
 */
export function useJDEliteGoods(p: IJdGoodsListModel) {
    // 是否處於加載狀態中（下拉）
    ...
    const getData = async () => {
        ...
        const res = await showJdjxByEliteId(p)
        loading.value = false
        if (res.resultCode === 1) {
