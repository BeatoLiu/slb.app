import { showMemberCommodityItem, showMemberCommodityModel } from "@/apis/model/mxModel"
 ...
 */
export function useGoodsList(p: showMemberCommodityModel) {
    // console.log(p)
    // 是否處於加載狀態中（下拉）
    ...
    // 接口返回數據列表
    const dataList = ref<showMemberCommodityItem[]>([])
    // 記錄總條數
    const total = ref(0)
    ...
    // 獲取數據
    const getData = async () => {
        let res = await showMemberCommodity(p)
        loading.value = false
        if (res.resultCode === 1) {
