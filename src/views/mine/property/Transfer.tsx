...
export default defineComponent({
	...
	setup() {
		...
		const dis = computed(() => {
			return !(
				params.amount.length &&
				+params.amount <= walletBalance.value.mwAmount &&
				/^0xzs00/.test(params.toChainAddr) &&
				params.toChainAddr.length === 40
			)
		})
		// 扫一扫
		const scan = () => {
				...
				transfer(params).then(async res => {
					if (res.resultCode === 1) {
						await store.dispatch('user/setWalletBalance', {
							...walletBalance.value,
							mwAmount: walletBalance.value.mwAmount - +params.amount
						})
						Toast('转账成功')
		...
		const tips = () => {
			...
			if (!(/^0xzs00/.test(params.toChainAddr) && params.toChainAddr.length === 40)) {
				...
				!(params.amount.length && +params.amount <= walletBalance.value.mwAmount && +params.amount > 0)
			) {
				return (
					<div style={DivStyle}>
