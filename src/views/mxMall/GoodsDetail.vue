		...
		<div class="pro-detail">
			...
			<div class="pro-detail-list" v-for="item in cSpuCollection" :key="item.pcCode">
				...
				<div class="val" v-for="ele in item.params" :key="ele.pcdSubCode">
					...
					<span v-if="item.pcSourceType == 4 || item.pcSourceType == 5">{{ ele.input }} {{ ele.pcdSubName }}</span>
				</div>
			</div>
		</div>
...
const Sku = defineAsyncComponent(() => import('../../components/Sku')) // from '../../components/Sku'
import { cDetailPicItem, csbListItem, prdCsSkuCollectionItem } from '../../apis/model/mxModel'
...
export default defineComponent({
	...
	setup() {
		...
		const cDetailPic = ref<cDetailPicItem[]>([])
		const cSpuCollection = ref<prdCsSkuCollectionItem[]>([])
		const csbList = ref<csbListItem[]>([])
		...
		const getSku = () => {
			...
			csbList.value.map((ele: any) => {
				...
				ele.csSkuCollection.map((mapItem: any) => {
		...
		const submitOrder = (obj: any) => {
			let poPayFactSum =
				data.poGroupType === 1 ? (obj.csWholesalePrice * obj.buyNum).toFixed(2) : (obj.csPrice1 * obj.buyNum).toFixed(2)
			data.show = false
			let params = {
		...
		const onBuyClicked = (data: any) => {
			console.log(data)
			data.show = false
			let id = data.selectedSkuComb.id
			// console.log(data.csbList)
...
.food {
	...
	.foot {
		...
		align-items: center;
		background: #fff;
		box-shadow: 0px -3px 4px 0px rgba(49, 50, 50, 0.21);
		.service,
		.car {
			width: 126 * @fontSize;
			text-align: center;
	...
	.fix-right {
		...
		.item {
			...
			height: 80 * @fontSize;
			background: #fff;
			box-shadow: 0px 1px 5px 0px rgba(49, 50, 50, 0.33);
