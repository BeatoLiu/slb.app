 ...
 * @returns
 */
export const updateMemberWJSConfirmLazyFlag = (p: IUpdateMemberWJSConfirmLazyFlagModel) =>
	httpGet<IBaseResT>(Api.UPDATE_MEMBER_WJS_CONFIRM_LAZY_FLAG, p)
 ...
 */
export const getExchangeRationDcToCny = (p: IGetExchangeRationDcToCnyModel) =>
	httpGet<IBaseResT>(Api.GET_EXCHANGE_RATION_DC_TO_CNY, p, { showLoading: false })
 ...
 */
export const saleSusdFromSlb = (p: ISaleSusdFromSlbModel) =>
	httpGet<IBaseResT>(Api.SALE_SUSD_FROM_SLB, p, { showLoading: false })

/**
 * @description 通过支付码参数获取商户信息
 */
export const getMerchantBymCodeAndMqcCode = (p: { id: string; mqcCode: string }) =>
	httpPost<IBaseResT>(Api.GET_MERCHANT_BY_M_CODE_AND_MQC_CODE, p, { showLoading: false })
 ...
 * @description 信用卡还款
 */
export const payCreditReturnForDc = (p: IPayCreditReturnForDcModel) =>
	httpPost<IBaseResT>(Api.PAY_CREDIT_RETURN_FOR_DC, p)

/**
 * @description  实时获取支付订单信息
 * @param p
 */
export const refreshGetPayOrder = (p: { orderCode: number }) =>
	httpPost<IBaseResT>(Api.REFRESH_GET_PAY_ORDER, p, { showLoading: false })
